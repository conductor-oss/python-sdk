...
Demonstrates:
    - Fan-out: one Orchestrator agent sending the same task to N Worker agents
      by sending a WMQ message once per worker, all from a single @tool
    ...
    - Unique tool names per worker: Conductor routes tasks by definition name,
      ...
      gets tools named submit_answer_<name>.
    - Spawn-safety: worker tools are backed by *module-level* callables (not
      closures over the runtime). Each rebuilds a workflow client from the
      ambient Configuration inside its spawned worker process. The IPC dir is
      shared across processes via an env var (a per-import tempfile.mkdtemp()
      would give every spawned worker its own dir).
    - Filesystem IPC:
...
# Workers run in *spawned* processes that re-import this module. Derive the IPC
# directory from an env var (set once in the parent, inherited by the spawned
# workers) so every process shares one directory — a per-import
# tempfile.mkdtemp() would give each worker its own dir and break the
# file-based synchronisation.
_ipc_dir = Path(
    os.environ.setdefault("FAN_OUT_FAN_IN_IPC_DIR", tempfile.mkdtemp(prefix="fan_out_fan_in_"))
)
...
# ---------------------------------------------------------------------------
# Spawn-safe worker callables — module-level (not closures over the runtime).
# Each rebuilds a workflow client from the ambient Configuration so it can push
# a WMQ message from inside its spawned worker process.
# ---------------------------------------------------------------------------
...
def _workflow_client():
    ...
    return OrkesClients(Configuration()).get_workflow_client()


class _SubmitAnswer:
    """Send this worker's answer to the Collector, then write a sentinel."""

    def __init__(self, worker_name: str, collector_id: str) -> None:
    ...
    def __call__(self, question: str, answer: str) -> str:
        ...
        _workflow_client().send_message(
            self.collector_id,
            {"question": question, "worker_name": self.worker_name, "answer": answer},
        )
        (_ANSWERS_DIR / f"{self.worker_name}_{time.time_ns()}.done").touch()
        return "submitted"


class _FanOut:
    """Broadcast a question to all worker agents simultaneously."""

    def __init__(self, worker_ids: list) -> None:
    ...
    def __call__(self, question: str) -> str:
        ...
        for wid in self.worker_ids:
            wf.send_message(wid, {"question": question})
        return f"broadcasted to {len(self.worker_ids)} workers"
...
def build_worker(worker_name: str, collector_id: str) -> Agent:
    ...
    submit_answer = tool(name=f"submit_answer_{worker_name}")(
        _SubmitAnswer(worker_name, collector_id)
    )
    ...
    try:
        with AgentRuntime() as runtime:
            ...
            for name in WORKER_NAMES:
                wh = runtime.start(
                    build_worker(name, collector_id),
