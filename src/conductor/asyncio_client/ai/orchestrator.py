...
class AsyncAIOrchestrator:
    ...
    async def add_prompt_template(self, name: str, prompt_template: str, description: str):
        await self.prompt_client.save_prompt(name, description, prompt_template)
        return self

    async def get_prompt_template(self, template_name: str) -> Optional[MessageTemplateAdapter]:
        try:
    ...
    async def add_vector_store(
        ...
        existing_integration = await self.integration_client.get_integration(db_integration_name)
        if existing_integration is None or overwrite:
            await self.integration_client.save_integration(db_integration_name, vector_db)
