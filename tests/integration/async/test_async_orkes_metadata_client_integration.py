...
class TestOrkesMetadataClientIntegration:
    ...
    @pytest.mark.v4_1_73
    @pytest.mark.asyncio
    async def test_workflow_lifecycle_simple(
        self,
        ...
        try:
            ...
            retrieved_workflow = await metadata_client.get_workflow_def(
            ...
            all_workflows = await metadata_client.get_all_workflow_defs()
    ...
    @pytest.mark.v4_1_73
    @pytest.mark.asyncio
    async def test_workflow_lifecycle_complex(
        self,
        metadata_client: OrkesMetadataClient,
        ...
        try:
            await metadata_client.register_workflow_def(
                complex_workflow_def, overwrite=True
            )
    ...
    @pytest.mark.v4_1_73
    @pytest.mark.asyncio
    async def test_workflow_versioning(
        self,
        metadata_client: OrkesMetadataClient,
        test_suffix: str,
        ...
        try:
            ...
            await metadata_client.register_workflow_def(workflow_v1, overwrite=True)
            await metadata_client.register_workflow_def(workflow_v2, overwrite=True)

            retrieved_v1 = await metadata_client.get_workflow_def(
                workflow_name, version=1
            )
            assert retrieved_v1.version == 1
            assert retrieved_v1.timeout_seconds == 60

            retrieved_v2 = await metadata_client.get_workflow_def(
                workflow_name, version=2
            )
            ...
            try:
                ...
                await metadata_client.unregister_workflow_def(workflow_name, 2)
            except Exception as e:
    ...
    async def test_workflow_update(
        ...
        try:
            ...
            await metadata_client.register_workflow_def(
                initial_workflow, overwrite=True
            )

            retrieved_workflow = await metadata_client.get_workflow_def(workflow_name)
            ...
            await metadata_client.update_workflow_def(updated_workflow, overwrite=True)

            updated_retrieved_workflow = await metadata_client.get_workflow_def(
                workflow_name
            )
    ...
    async def test_task_lifecycle(
        ...
        try:
            ...
            retrieved_task = await metadata_client.get_task_def(simple_task_def.name)
            ...
            all_tasks = await metadata_client.get_all_task_defs()
            ...
            try:
                await metadata_client.unregister_task_def(simple_task_def.name)
            except Exception as e:
                print(
    ...
    async def test_task_update(
        ...
        try:
            ...
            await metadata_client.register_task_def(initial_task)

            retrieved_task = await metadata_client.get_task_def(task_name)
            ...
            await metadata_client.update_task_def(updated_task)

            updated_retrieved_task = await metadata_client.get_task_def(task_name)
    ...
    async def test_workflow_tags(
        ...
        try:
            ...
            await metadata_client.register_workflow_def(workflow, overwrite=True)
            ...
            for tag in tags:
                await metadata_client.add_workflow_tag(tag, workflow_name)

            retrieved_tags = await metadata_client.get_workflow_tags(workflow_name)
            ...
            await metadata_client.delete_workflow_tag(tag_to_delete, workflow_name)

            retrieved_tags_after_delete = await metadata_client.get_workflow_tags(
                workflow_name
            )
            remaining_tag_keys = [tag.key for tag in retrieved_tags_after_delete]
            assert "priority" not in remaining_tag_keys

            await metadata_client.set_workflow_tags(tags, workflow_name)

            final_tags = await metadata_client.get_workflow_tags(workflow_name)
    ...
    async def test_task_tags(
        ...
        try:
            ...
            await metadata_client.register_task_def(task)

            tags = [
                MetadataTag(key="category", value="data_processing", type="METADATA"),
                MetadataTag(key="team", value="backend", type="METADATA"),
                MetadataTag(key="criticality", value="medium", type="METADATA"),
            ]

            for tag in tags:
                await metadata_client.add_task_tag(tag, task_name)

            retrieved_tags = await metadata_client.get_task_tags(task_name)
            assert len(retrieved_tags) >= 3
            tag_keys = [tag.key for tag in retrieved_tags]
            ...
            tag_to_delete = MetadataTag(
                key="criticality", value="medium", type="METADATA"
            )
            await metadata_client.delete_task_tag(tag_to_delete, task_name)

            retrieved_tags_after_delete = await metadata_client.get_task_tags(task_name)
            remaining_tag_keys = [tag.key for tag in retrieved_tags_after_delete]
            assert "criticality" not in remaining_tag_keys

            await metadata_client.set_task_tags(tags, task_name)

            final_tags = await metadata_client.get_task_tags(task_name)
            final_tag_keys = [tag.key for tag in final_tags]
    ...
    @pytest.mark.v4_1_73
    @pytest.mark.asyncio
    async def test_metadata_not_found(self, metadata_client: OrkesMetadataClient):
        non_existent_workflow = f"non_existent_{str(uuid.uuid4())}"
        non_existent_task = f"non_existent_{str(uuid.uuid4())}"

        with pytest.raises(ApiException) as exc_info:
            await metadata_client.get_workflow_def(non_existent_workflow)
        assert exc_info.value.status == 404

        with pytest.raises(ApiException) as exc_info:
            await metadata_client.unregister_workflow_def(non_existent_workflow, 1)
        assert exc_info.value.status == 404

        with pytest.raises(ApiException) as exc_info:
            await metadata_client.get_task_def(non_existent_task)
        assert exc_info.value.status == 404

        with pytest.raises(ApiException) as exc_info:
            await metadata_client.unregister_task_def(non_existent_task)
        assert exc_info.value.status == 404

    @pytest.mark.v5_2_6
    @pytest.mark.v4_1_73
    @pytest.mark.asyncio
    async def test_complex_metadata_management_flow(
        ...
        try:
            ...
            for workflow_type, description in workflow_types.items():
                ...
                tags = [
                    MetadataTag(key="type", value=workflow_type, type="METADATA"),
                    MetadataTag(key="environment", value="test", type="METADATA"),
                    MetadataTag(key="owner", value="integration_test", type="METADATA"),
                ]
            ...
            for task_type, description in task_types.items():
                ...
                tags = [
                    MetadataTag(key="category", value=task_type, type="METADATA"),
                    MetadataTag(key="team", value="backend", type="METADATA"),
                    MetadataTag(key="criticality", value="medium", type="METADATA"),
                ]
    ...
    async def _perform_comprehensive_cleanup(
        ...
        for workflow_name, version in created_resources["workflows"]:
            try:
                await metadata_client.get_workflow_def(workflow_name, version=version)
        ...
        for task_name in created_resources["tasks"]:
            try:
                await metadata_client.get_task_def(task_name)
